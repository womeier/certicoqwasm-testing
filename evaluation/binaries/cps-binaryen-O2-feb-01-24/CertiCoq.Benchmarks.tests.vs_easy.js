const fs = require('fs');
const bytes = fs.readFileSync(__dirname + '/CertiCoq.Benchmarks.tests.vs_easy.wasm');
...
(async () => {
    ...
    try {
        ...
        console.log("Benchmark CertiCoq.Benchmarks.tests.vs_easy.wasm took " + (stop - start) + "ms.");
    } catch (error) {
        console.log(error);
        process.exit(1);
    }
