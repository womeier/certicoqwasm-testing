const fs = require('fs');
const bytes = fs.readFileSync(__dirname + '/CertiCoq.Benchmarks.tests.vs_hard.wasm');
...
(async () => {
    ...
    try {
        ...
        console.log("Benchmark CertiCoq.Benchmarks.tests.vs_hard.wasm took " + (stop - start) + "ms.");
    } catch (error) {
        console.log(error);
        process.exit(1);
    }
