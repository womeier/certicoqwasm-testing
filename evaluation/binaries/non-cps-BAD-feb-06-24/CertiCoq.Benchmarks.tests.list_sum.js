const fs = require('fs');
const bytes = fs.readFileSync(__dirname + '/CertiCoq.Benchmarks.tests.list_sum.wasm');
...
(async () => {
    ...
    try {
        ...
        console.log("Benchmark CertiCoq.Benchmarks.tests.list_sum.wasm took " + (stop - start) + "ms.");
    } catch (error) {
        console.log(error);
        process.exit(1);
    }
