const fs = require('fs');
const bytes = fs.readFileSync(__dirname + '/CertiCoq.Benchmarks.tests.sha_fast.wasm');
...
(async () => {
    ...
    try {
        ...
        console.log("Benchmark CertiCoq.Benchmarks.tests.sha_fast.wasm took " + (stop - start) + "ms.");
    } catch (error) {
        console.log(error);
        process.exit(1);
    }
